...
fill_region <- function(x, y) {
  if (x >= 22 && y <= 7.8 || x <= -22 && y <=7.8){
    shotData$shot_region[row] <- "C3"
  } else if(y > 23.5){
    shotData$shot_region[row] <-"NC3"
  } else {
    shotData$shot_region[row] <- "2PT"
  }
}
...
results$`%_eFG_C3`[1] <- round((0 + .5*nrow(made3CptA)) / nrow(threePCA), digits = 4)
results$`%_eFG_C3`[2] <- round((0 + .5*nrow(made3CptB)) / nrow(threePCB), digits = 4)
#field goal percentage 3pt no corner
made3NCptA <- subset(threePNCA , fgmade == 1)
made3NCptB <- subset(threePNCA , fgmade == 1)
results$`%_eFG_NC3`[1] <- round((0 + .5*nrow(made3NCptA)) / nrow(threePNCA), digits = 4)
results$`%_eFG_NC3`[2] <- round((0 + .5*nrow(made3NCptB)) / nrow(threePNCB), digits = 4)
###Answer 
print(results)
